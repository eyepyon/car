 ...
 * 有効なBase64文字列を生成
 */
const base64StringArbitrary = (minLength = 100, maxLength = 1000): fc.Arbitrary<string> =>
  fc.array(
 ...
 * 有効なデータURLを生成
 */
const dataUrlArbitrary = (minLength = 100, maxLength = 1000): fc.Arbitrary<string> =>
  fc.record({
 ...
 * 有効な画像サイズを生成
 */
const imageDimensionsArbitrary = (): fc.Arbitrary<{ width: number; height: number }> =>
  fc.record({
...
describe('Image Optimizer Property Tests', () => {
      ...
      fc.assert(
        fc.property(
          ...
          ({ width, height }, maxWidth, maxHeight) => {
            const result = calculateResizedDimensions(width, height, maxWidth, maxHeight);
      ...
      fc.assert(
        fc.property(
          ...
          (width, height, maxWidth, maxHeight) => {
            ...
            const ratioDiff = Math.abs(originalRatio - newRatio) / originalRatio;
      ...
      fc.assert(
        fc.property(
          ...
          (currentSize, targetSize, currentQuality) => {
            const estimated = estimateOptimalQuality(currentSize, targetSize, currentQuality);
  ...
  describe('最適化判定', () => {
    it('最大ファイルサイズを超える画像は最適化が必要と判定される', () => {
      fc.assert(
        fc.property(
          fc.integer({ min: 1024, max: 100000 }),
          (maxFileSize) => {
      ...
      fc.assert(
        fc.property(
          ...
          (originalSize, quality) => {
            const estimated = optimizer.estimateOptimizedSize(originalSize, quality);
      ...
      fc.assert(
        fc.property(
          ...
          (originalSize, lowQuality, highQuality) => {
            const lowEstimate = optimizer.estimateOptimizedSize(originalSize, lowQuality);
            const highEstimate = optimizer.estimateOptimizedSize(originalSize, highQuality);
