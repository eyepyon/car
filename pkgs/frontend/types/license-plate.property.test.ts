 ...
 */
const serialNumberArbitrary = fc.integer({ min: 1, max: 9999 }).map((n) => n.toString());
...
const partialLicensePlateDataArbitrary = fc.record({
  ...
  classificationNumber: fc.option(classificationNumberArbitrary, { nil: undefined }),
    ...
    fc.assert(
      fc.property(validLicensePlateDataArbitrary, (data) => {
        ...
        const expectedManual =
          data.region + data.classificationNumber + data.hiragana + data.serialNumber;
