...
import { validateImage, validateImageSync, validateResolution } from './image-validator';
import {
...
const createMockImage = (overrides?: Partial<CapturedImage>): CapturedImage => ({
...
describe('統合テスト: 画像検証フロー', () => {
  ...
  it('validateImageSyncは解像度チェックを実行する', () => {
    ...
    expect(invalidResult.errors.some((e) => e.code === 'RESOLUTION')).toBe(true);
  });
});
...
describe('統合テスト: 完全なフロー', () => {
  ...
  it('リアルタイムモードでの重複抑制フロー', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const result = await client.recognize('base64image', { mode: 'realtime' });
...
describe('統合テスト: データ構造の整合性', () => {
  ...
  it('全てのプレートタイプが有効', () => {
    const validTypes = ['REGULAR', 'LIGHT', 'COMMERCIAL', 'RENTAL', 'DIPLOMATIC'];
